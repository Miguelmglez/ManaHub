...
//
//  KMP migration note (Deck Engine Unification plan, RUN 5 / D5): this object moved from
//  `:app`'s `com.mmg.manahub.core.tagging` package into `commonMain` here, alongside the
//  [StrategyAnalyzer]/[KeywordAnalyzer]/[TypeLineAnalyzer]/[GameChangerAnalyzer] analyzers that
//  already made the same move earlier in the KMP migration (see `core/tagging/TagAnalyzers.kt` in
//  `:app`, whose own doc comment calls out "TagDictionary, which is JVM-only" as the reason
//  [StrategyAnalyzer] became a constructor-injected class). The ONLY genuinely JVM-only piece was
//  [localize] (needs `java.util.Locale` + `:app`'s `CardTypeTranslator`) — that single function
//  stays behind in `:app`'s compat shim (`core/tagging/TagDictionary.kt`) as an extension function
//  built on this object's public API ([get]), so `TagDictionary.localize(tag)` keeps compiling
//  unchanged at every existing call site. Everything else here was ALREADY pure Kotlin.
//
//  This move is what lets `:tools:tag-pipeline` (a plain-JVM CLI, Deck Engine Unification plan D5)
//  reuse the EXACT SAME base dictionary + detection rules the in-app tagging engine uses — zero
//  drift, the whole point of D5's "reuse the production rule engine" requirement.
//
...
//     "lifegain").
//
//  User overrides are loaded at app start by `:app`'s `TagDictionaryRepository` and merged on top
//  via [applyOverrides]. The Settings → Tag Dictionary screen edits those overrides directly. The
//  offline tag pipeline (`:tools:tag-pipeline`) NEVER calls [applyOverrides] — it only reads [all]
//  (the base system dictionary), since bulk per-oracle_id tags are a system-wide computation, not
//  scoped to any one user's overrides.
//
//  Lookups:
//   - get(key) / all()           → raw entries, no localization
//   - findKeysByTerm("Flying")   → ["flying"]   (reverse, label/key search)
//
//  TYPE-category tags (creature subtypes etc.) are *not* stored here; `:app`'s
//  `TagDictionary.localize` delegates those to `CardTypeTranslator` so there is a single source of
//  truth for type translations (an Android-only concern, out of scope for commonMain).
// ═══════════════════════════════════════════════════════════════════════════════
...
/**
 * Persisted user override applied on top of [TagDictionary]'s base entries.
 *
