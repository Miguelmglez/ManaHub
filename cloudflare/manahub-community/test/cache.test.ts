...
import { classifyFreshness, AGGREGATE_TTL_MS, STALE_WHILE_REVALIDATE_MS } from "../src/lib/cache";
...
describe("classifyFreshness (stale-while-revalidate)", () => {
  ...
  it("is fresh within the TTL", () => {
    ...
    expect(classifyFreshness({ data: {}, cachedAt }, cachedAt + AGGREGATE_TTL_MS - 1)).toBe("fresh");
  });

  it("is stale within the SWR grace window past the TTL", () => {
    const now = cachedAt + AGGREGATE_TTL_MS + 1;
    expect(classifyFreshness({ data: {}, cachedAt }, now)).toBe("stale");
    const edge = cachedAt + AGGREGATE_TTL_MS + STALE_WHILE_REVALIDATE_MS - 1;
    expect(classifyFreshness({ data: {}, cachedAt }, edge)).toBe("stale");
  });

  it("is expired past the SWR grace window", () => {
    const now = cachedAt + AGGREGATE_TTL_MS + STALE_WHILE_REVALIDATE_MS + 1;
