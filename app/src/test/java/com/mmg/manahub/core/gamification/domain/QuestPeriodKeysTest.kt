...
class QuestPeriodKeysTest {
    ...
    fun `daily key is the ISO local date`() {
        assertEquals("2026-06-12", QuestPeriodKeys.dailyKey(LocalDate.of(2026, 6, 12)))
    }

    @Test
    fun `weekly key is ISO week-based year and week, zero-padded`() {
        // 2026-06-12 is a Friday in ISO week 24 of 2026.
        assertEquals("2026-W24", QuestPeriodKeys.weeklyKey(LocalDate.of(2026, 6, 12)))
        ...
        assertEquals("2026-W02", QuestPeriodKeys.weeklyKey(LocalDate.of(2026, 1, 5)))
    }

    @Test
    fun `previous day and week helpers step back one period`() {
        assertEquals("2026-06-11", QuestPeriodKeys.previousDailyKey(LocalDate.of(2026, 6, 12)))
        assertEquals("2026-W23", QuestPeriodKeys.previousWeeklyKey(LocalDate.of(2026, 6, 12)))
    }
    ...
    fun `ISO week-based year differs from calendar year at the year boundary`() {
        ...
        assertEquals("2026-W53", QuestPeriodKeys.weeklyKey(LocalDate.of(2026, 12, 31)))
        // 2027-01-01 is a Friday → still ISO week 53 of week-based-year 2026 (NOT 2027-W01).
        assertEquals("2026-W53", QuestPeriodKeys.weeklyKey(LocalDate.of(2027, 1, 1)))
        // 2027-01-04 is the Monday that starts ISO week 1 of 2027.
        assertEquals("2027-W01", QuestPeriodKeys.weeklyKey(LocalDate.of(2027, 1, 4)))
    }
    ...
    fun `dates in the same ISO week map to the same weekly key`() {
        ...
        for (offset in 0..6) {
            assertEquals("2026-W24", QuestPeriodKeys.weeklyKey(monday.plusDays(offset.toLong())))
        }
    }
    ...
    fun `daily expiry is the start of the next local day`() {
        ...
        val expected = date.plusDays(1).atStartOfDay(utc).toInstant().toEpochMilli()
    ...
    fun `weekly expiry is the Monday that begins the next ISO week`() {
        ...
        val date = LocalDate.of(2026, 6, 12)
        val expectedMonday = LocalDate.of(2026, 6, 15).atStartOfDay(utc).toInstant().toEpochMilli()
    ...
    fun `weekly expiry from a Monday advances a full week`() {
        ...
        val monday = LocalDate.of(2026, 6, 8)
        val expectedNextMonday = LocalDate.of(2026, 6, 15).atStartOfDay(utc).toInstant().toEpochMilli()
