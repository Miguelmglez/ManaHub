...
class DefaultDraftEngineTest {
    ...
    private val fakeBot = object : BotDrafter {
        override fun pick(seat: DraftSeat, pack: BoosterPack, round: Int, pickNumber: Int) =
            pack.cards.first()
    }
    ...
    private fun runFullDraft(): com.mmg.manahub.feature.draft.domain.model.DraftState {
        ...
        while (!engine.isComplete(state)) {
            ...
            state = engine.applyHumanPick(state, firstCard.card.scryfallId)
        }
        return state
    }
    ...
    fun passDirectionAlternatesAfterRound1() {
        ...
        repeat(packSize) {
            ...
            state = engine.applyHumanPick(state, card.card.scryfallId)
        }
    ...
    fun autoPick_selectsLowestRank() {
        ...
        val after = engine.autoPick(state)
