 ...
 *     next cycle). Watermarks advance ONLY after a fully successful cycle.
 *
...
class GamificationSyncManager @Inject constructor(
    ...
    private suspend fun runSync(userId: String): Result<Unit> = runCatching {
        ...
        // MAX(id) covers pre-existing local rows AND rows just pulled (which got fresh local ids), so
        // pulled rows are never echoed back on the next push.
        syncPrefs.saveGamificationPushedLedgerId(userId, gamificationDao.getMaxLedgerId())
        syncPrefs.saveGamificationSyncMillis(userId, syncStartTime)
        Unit
    }.onFailure { error ->
