...
/**
 * Sealed interface representing the icon for a first step.
 * Can be either a Material [ImageVector] or a local [drawable] resource.
 */
sealed interface StepIcon {
    data class Vector(val imageVector: ImageVector) : StepIcon
    data class Drawable(val resId: Int) : StepIcon
}
 ...
 * @param icon      Icon rendered at the top of the slide ([StepIcon]).
...
val ALL_FIRST_STEPS: List<FirstStepItem> = listOf(
   ...
   /* FirstStepItem(
    ...
    FirstStepItem(
        ...
        icon = StepIcon.Vector(Icons.Default.Star),
        action = HomeAction.RateApp,
    ),
)
