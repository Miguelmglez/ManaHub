...
- Session codes are **6-digit numeric** (`^[0-9]{6}$`). `disconnect()` MUST `unsubscribe()` +
  `removeChannel()` (2026-09-22): supabase-kt caches channels by topic, so a leftover joined channel makes
  the next `connect()` throw. The old "never removeChannel" rule came from a misdiagnosis — the real cause
  of "Engine doesn't support WebSocketCapability" was the Ktor `Android` engine in `SupabaseModule`; the
  Supabase client MUST use `OkHttp.create()`.
...
- Connect order in game: disconnect stale → connect → snapshot → collect. `connect()` times out after 10 s
  and throws a typed `RealtimeConnectException`; every failure unregisters the handle. `broadcast*` never
  throw — they return `false`. Unknown server enum values map to `UNKNOWN` (non-terminal), never to
  `ABANDONED`/`LEFT`.
