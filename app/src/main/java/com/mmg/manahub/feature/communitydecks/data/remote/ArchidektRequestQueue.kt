 ...
 * conservative: at least [MIN_DELAY_MS] elapses between consecutive requests
 * (≈5 req/s). The [Mutex] serialises concurrent callers; the inter-request delay is
 * applied inside the lock so only one coroutine proceeds at a time.
 *
 * Mirrors [com.mmg.manahub.core.network.ScryfallRequestQueue] but with Archidekt-tuned
 * constants ([MIN_DELAY_MS], [MAX_RETRIES], [INITIAL_BACKOFF_MS]).
 *
 * ## Retry / back-off
 * When the server responds with HTTP 429 (Too Many Requests) or 503
 * (Service Unavailable), [execute] retries the call up to [MAX_RETRIES] times using
 * truncated binary-exponential back-off with ±25 % random jitter and a hard cap of
 * [MAX_BACKOFF_MS]. If the response includes a `Retry-After` header (integer seconds),
 * that value overrides the computed back-off for that attempt.
 *
...
class ArchidektRequestQueue @Inject constructor() {
     ...
     * 2. Truncated binary-exponential back-off with ±25 % jitter, capped at [MAX_BACKOFF_MS].
     */
    private fun computeBackoffMs(attempt: Int, exception: HttpException): Long {
        ...
        // Binary-exponential: 500 ms, 1000 ms, … capped at MAX_BACKOFF_MS.
        val base = INITIAL_BACKOFF_MS shl attempt          // 500 * 2^attempt
    ...
    companion object {
        /** Minimum gap between consecutive Archidekt requests (≈5 req/s, conservative). */
