...
--
-- Dedicated, keyset-paginated search over ONE of a friend's lists (collection /
-- wishlist / trade) with Scryfall-like metadata filters from card_search_index
-- (see 20260922_card_search_index.sql). get_friend_collection stays untouched
-- (TradeProposalViewModel depends on it).
--
-- Access: same tokens and rules as get_friend_collection (friendship or the
-- per-list public flag). SECURITY INVOKER: row visibility is still enforced by
-- the list tables' RLS, which today admits only the owner and mutual friends,
-- so a public-flag-only viewer passes the check but receives zero rows (same
-- de-facto behaviour as get_friend_collection).
--
-- Cursor: ORDER BY (sort_key COLLATE "C", row_id). sort_key = '0' || lower(name)
-- for hydrated cards, '1' || scryfall_id for unhydrated ones (sorted last).
-- Next page: pass the last row's sort_key + row_id as p_after_sort_key /
-- p_after_row_id with identical filters. has_more=false marks the last page.
--
-- Caps (typed INVALID_ARGUMENT error, never silent truncation): text params and
-- array elements <= 100 chars, arrays <= 50 elements, page size clamped to 1..100.
--
-- SUPERSEDED by 20260922_friend_search_hardening.sql (v2: unindexed_count column,
-- 4xx errcodes, PROFILE_INCOMPLETE, type/format arrays capped at 10). Kept as the
-- historical v1 that was applied first.
--
...
BEGIN
  ...
  IF p_friend_user_id IS NULL OR p_friend_user_id = v_me THEN
    RAISE EXCEPTION 'SELF_LOOKUP' USING ERRCODE = 'P0003';
  END IF;
  IF p_list IS NULL OR p_list NOT IN ('collection', 'wishlist', 'trade') THEN
    RAISE EXCEPTION 'INVALID_LIST' USING ERRCODE = 'P0004';
  END IF;
  ...
  IF COALESCE(cardinality(p_types_all), 0)     > c_max_array
     OR COALESCE(cardinality(p_types_any), 0)     > c_max_array
     OR COALESCE(cardinality(p_types_exclude), 0) > c_max_array
     ...
     OR COALESCE(cardinality(p_formats), 0)       > c_max_array
  ...
  -- ---- access (same rules/tokens as get_friend_collection) ----------------
  SELECT EXISTS (
  ...
  IF NOT COALESCE(v_is_friend, false) AND NOT COALESCE(v_list_public, false) THEN
    RAISE EXCEPTION 'ACCESS_DENIED' USING ERRCODE = 'P0005';
  END IF;

  -- ---- normalized filters (LIKE wildcards escaped; default escape char is '\') ----
  ...
  SELECT p.sl, p.rid, p.sid, p.qty, p.foil, p.cond, p.lng, p.cname, p.cset, p.crarity,
         p.skey::text, (count(*) OVER ()) > v_limit
    FROM page p
